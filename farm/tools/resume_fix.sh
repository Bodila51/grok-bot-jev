#!/bin/bash
# Manual follow-up on a codex session (farm.py run does this automatically for verification fixes).
# usage: resume_fix.sh <job_id> <session_id> <prompt_file> <image|-> <logname> [model=gpt-6-sol] [seat=codex-sub]
set -u; F=${FARM_HOME:-$(cd "$(dirname "$0")/.." && pwd)}; J=$F/jobs/$1; M=${6:-gpt-6-sol}; S=${7:-codex-sub}
PY=python3; IMG=(); [ "$4" != "-" ] && IMG=(-i "$4")
$PY -c "import sqlite3;c=sqlite3.connect('$F/farm.db');c.execute(\"UPDATE seats SET status='busy' WHERE id='$S'\");c.commit()"
cd "$J" && env -u OPENAI_API_KEY -u FARM_OPENAI_API_KEY -u ANTHROPIC_API_KEY -u TYPESAFE_API_KEY CODEX_HOME=$F/seats/$S/codex \
