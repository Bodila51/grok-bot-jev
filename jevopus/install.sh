#!/usr/bin/env bash
# Jevopus installer v2.6.20260926 (from a repo clone). Idempotent; safe to re-run. Never prints secrets.
...
[ "$(cd "$JEVOPUS_HOME" && pwd)" = "$SRC_DIR" ] || (cd "$SRC_DIR" && tar -cf - jevopus.py README.md tools/resume_fix.sh jevopuslib/__init__.py jevopuslib/bestof2.py jevopuslib/core.py jevopuslib/evidence.py jevopuslib/guide.py jevopuslib/jev.py jevopuslib/limits.py jevopuslib/models.py jevopuslib/reports.py jevopuslib/runner.py jevopuslib/setup.py recipes/assets/motion_reference.py recipes/code-review.json recipes/competitor-analysis.json recipes/data-cleanup.json recipes/landing-page.json recipes/lead-finder.json recipes/media-kit.json recipes/motion-video.json recipes/presentation.json recipes/research-brief.json recipes/x-post-drafts.json | tar -xf - -C "$JEVOPUS_HOME") || die "copy failed"
chmod +x "$JEVOPUS_HOME/jevopus.py" "$JEVOPUS_HOME/tools/resume_fix.sh"
echo "$JEVOPUS_JEV_DIR" > "$JEVOPUS_HOME/.jev_dir"
echo "installed Jevopus v2.6.20260926 ($(find "$JEVOPUS_HOME/jevopuslib" "$JEVOPUS_HOME/recipes" -type f | wc -l) lib/recipe files); config.json, jevopus.db, jobs/ and seats/ are kept"
