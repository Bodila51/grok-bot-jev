#!/usr/bin/env python3
"""Jevopus v2.5 (formerly Farm): worker router. Jev (TypeSafe) judges; code owns leases, runs, policy. Secrets never printed.
...
try:
    ...
    if len(sys.argv) < 2 or sys.argv[1] not in ("doctor", "init", "status", "recipes", "limits", "jev-mode"):
        sys.exit("Jev router venv not found (need grok-bot-jev/.venv with typesafe-sdk). Run jevopus_install.sh or jevopus.py doctor.")

from jevopuslib import limits, reports, runner, setup  # noqa: E402
...
def main():
    ...
    for n in ("init", "tick", "status", "weekly", "recipes", "doctor", "limits"): sp.add_parser(n)
    ...
    {"init": lambda a: (db(), config(), print(f"db ready: {DB}; config: {len(config()['models'])} models")), "submit": runner.cmd_submit, "route": runner.cmd_route,
     ...
     "setup-seat": setup.cmd_setup_seat, "limits": limits.cmd_limits, "jev-mode": reports.cmd_jev_mode}[a.cmd](a)
